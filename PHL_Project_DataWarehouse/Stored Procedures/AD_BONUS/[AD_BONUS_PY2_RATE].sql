...
/****** Object:  StoredProcedure [dbo].[AD_BONUS_PY2_RATE]    Script Date: 07/10/2022 11:07:26 ******/
SET ANSI_NULLS ON
GO
...
, A AS (
	SELECT
		...
		,DATEADD(month, 2, [Due]) AS REPORT_MONTH
	...
	GROUP BY [Due], [Sales_Unit_Code], [Sales Unit]
)
, A0 AS (
	SELECT
		A02.TD,
	...
	WHERE A01.Due BETWEEN DATEADD(month, -7, @varREPORT_MONTH) AND DATEADD(month, -2, @varREPORT_MONTH)
)
, B0 AS (
	SELECT
		ZD AS AD_CODE_FINAL,
