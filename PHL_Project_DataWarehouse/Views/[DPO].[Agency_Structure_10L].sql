...
/****** Object:  View [DPO].[Agency_Structure_10L]    Script Date: 30/09/2022 16:28:18 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [DPO].[Agency_Structure_10L] AS
WITH A AS (
SELECT --A0.AD_Office 
	  --,A0.AD_Code
	  --,
...
SELECT 
	...
	, K.Supervisor_Code AS L1, A1.GRADE AS L1G
...
SELECT --D.*
	...
	, CASE
		WHEN D.[GRADE] = 'GM' THEN D.[Agent_Number]
		WHEN D.L1G = 'GM' THEN D.L1
		WHEN D.L2G = 'GM' THEN D.L2
		WHEN D.L3G = 'GM' THEN D.L3
		WHEN D.L4G = 'GM' THEN D.L4
		WHEN D.L5G = 'GM' THEN D.L5
		WHEN D.L6G = 'GM' THEN D.L6
		WHEN D.L7G = 'GM' THEN D.L7
		WHEN D.L8G = 'GM' THEN D.L8
		WHEN D.L9G = 'GM' THEN D.L9
		WHEN D.L10G = 'GM' THEN D.L10
	END AS GM
	, CASE
		WHEN D.[GRADE] = 'RM' THEN D.[Agent_Number]
		WHEN D.L1G = 'RM' THEN D.L1
		WHEN D.L2G = 'RM' THEN D.L2
		WHEN D.L3G = 'RM' THEN D.L3
		WHEN D.L4G = 'RM' THEN D.L4
		WHEN D.L5G = 'RM' THEN D.L5
		WHEN D.L6G = 'RM' THEN D.L6
		WHEN D.L7G = 'RM' THEN D.L7
		WHEN D.L8G = 'RM' THEN D.L8
		WHEN D.L9G = 'RM' THEN D.L9
		WHEN D.L10G = 'RM' THEN D.L10
	END AS RM

	, CASE
		WHEN D.[GRADE] = 'DM' THEN D.[Agent_Number]
		WHEN D.L1G = 'DM' THEN D.L1
		WHEN D.L2G = 'DM' THEN D.L2
		WHEN D.L3G = 'DM' THEN D.L3
		WHEN D.L4G = 'DM' THEN D.L4
		WHEN D.L5G = 'DM' THEN D.L5
		WHEN D.L6G = 'DM' THEN D.L6
		WHEN D.L7G = 'DM' THEN D.L7
		WHEN D.L8G = 'DM' THEN D.L8
		WHEN D.L9G = 'DM' THEN D.L9
		WHEN D.L10G = 'DM' THEN D.L10
	END AS DM
	, CASE
		WHEN D.[GRADE] = 'FM' THEN D.[Agent_Number]
		WHEN D.L1G = 'FM' THEN D.L1
		WHEN D.L2G = 'FM' THEN D.L2
		WHEN D.L3G = 'FM' THEN D.L3
		WHEN D.L4G = 'FM' THEN D.L4
		WHEN D.L5G = 'FM' THEN D.L5
		WHEN D.L6G = 'FM' THEN D.L6
		WHEN D.L7G = 'FM' THEN D.L7
		WHEN D.L8G = 'FM' THEN D.L8
		WHEN D.L9G = 'FM' THEN D.L9
		WHEN D.L10G = 'FM' THEN D.L10
	END AS FM
	--,D.[ID_AD]
	--,D.[Date_Appointed]
	--,D.[Terminated_date]
	...
	,D.[L0R]
	,D.[INDEX_LEADER]
	, IIF(D.GRADE <> D.L1G AND L1G <> 'IC', 1, 0) AS [L1 Direct]
	, IIF(D.L1G <> D.L2G AND L2G <> 'IC', 1, 0) AS [L2 Direct]
	, IIF(D.L2G <> D.L3G AND L3G <> 'IC', 1, 0) AS [L3 Direct]
	, IIF(D.L3G <> D.L4G AND L4G <> 'IC', 1, 0) AS [L4 Direct]
	, IIF(D.L4G <> D.L5G AND L5G <> 'IC', 1, 0) AS [L5 Direct]
	, IIF(D.L5G <> D.L6G AND L6G <> 'IC', 1, 0) AS [L6 Direct]
	, IIF(D.L6G <> D.L7G AND L7G <> 'IC', 1, 0) AS [L7 Direct]
	, IIF(D.L7G <> D.L8G AND L8G <> 'IC', 1, 0) AS [L8 Direct]
	, IIF(D.L8G <> D.L9G AND L9G <> 'IC', 1, 0) AS [L9 Direct]
	, IIF(D.L9G <> D.L10G AND L10G <> 'IC', 1, 0) AS [L10 Direct]
	--, AD.*
	----, CONCAT(D.[Agent_Number], FORMAT( D.ExplodedDate, 'yyyyMM') ) AS ID_AGENT
	--, CONCAT(AGENT.Sales_Unit_Code, FORMAT( D.ExplodedDate, 'yyyyMM') ) AS ID_AD
FROM D
--WHERE D.[ExplodedDate] = '2021-08-01'
...
SELECT A.[Agent_Number]
     ...
     ,IIF(A.[Curr_From] > B.[Appointed_TAPSU] AND A.[RN] = 1, CONVERT(CHAR(6),B.[Appointed_TAPSU], 112),CONVERT(CHAR(6),A.[Curr_From], 112)) AS EFFECTIVE
  ...
  FROM [PowerBI].[DPO].[Main_AGENT_AD_HISTORY] AS A
	...
	LEFT JOIN [PowerBI].[DPO].[Main_AGENT_INFO_DA] AS B
	ON A.Agent_Number = B.Agent_Number
	--WHERE A.[Agent_Number] = '60047382'
	--ORDER BY Curr_From
)
, AGENT_AD_Temp2  AS (
SELECT A.[Agent_Number]
	 ...
	 ,IIF(RN = 1, A.EFFECTIVE, LAG (CONVERT(CHAR(6),A.Curr_From, 112)) OVER (PARTITION BY A.Agent_Number ORDER BY A.[RN])) AS FROM_EFFECTIVE
...
, FINAL AS (
SELECT 
	IIF(Current_Structure.Sales_Unit_Code IS NULL,'ADNUL',Current_Structure.Sales_Unit_Code)  + CONVERT(CHAR(6), EOMONTH(GETDATE()),112) AS ID_AD_Current
	...
	--, S.Terminated_date
FROM E
LEFT JOIN [PowerBI].[DPO].[AGENCY_SFC] AS S
ON E.ID = S.ID_AGENT

LEFT JOIN AGENT_AD AS HISTORY
ON E.[Agent_Number] = HISTORY.Agent_Number
AND E.[ExplodedDate] >= HISTORY.FROM_EFFECTIVE AND E.[ExplodedDate] < IIF(HISTORY.END_EFFECTIVE IS NULL, '209912', HISTORY.END_EFFECTIVE)
