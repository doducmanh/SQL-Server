...
/****** Object:  View [DPO].[AD_STRUCTURE_UPLOAD_TO_S_DRIVE]    Script Date: 06/01/2023 12:56:41 ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [DPO].[AD_STRUCTURE_UPLOAD_TO_S_DRIVE] AS
...
SELECT 
      ...
      ,[Last_Status_Date]
      ,[DemotePromote_Date]
FROM [PowerBI].[DPO].[AD_STRUCTURE] AS A
LEFT JOIN [Main_TERRITORIES] AS B
ON A.Territory_Code = B.CODE
WHERE 
A.[ExplodedDate] IN (SELECT MAX([ExplodedDate]) FROM [PowerBI].[DPO].[AD_STRUCTURE])
...
, T_AD_OFFICES AS (
	...
	WHERE ID IN (SELECT MAX([ID]) AS ID FROM [PowerBI].[DPO].[Main_AD_STRUCTURE] GROUP BY [AD_Code])
	AND [AD_Code] NOT IN (SELECT DISTINCT [Sales_Unit_Code] FROM [PowerBI].[DPO].[Main_AGENT_INFO_DA])
)
, T_AD_OFFICES_2 AS (
	SELECT C.[Area_Name], C.[Sales_Unit_Code], D.Office_Name
	FROM T_AD_OFFICES AS C LEFT JOIN [PowerBI].[DPO].[DW_Office] AS D
...
LEFT JOIN [DPO].[AD Info]
ON T_LATEST_AD_STRUCTURE.AD_Code = [AD Info].[AD Code]
