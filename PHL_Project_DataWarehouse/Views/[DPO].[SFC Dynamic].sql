...
/****** Object:  View [DPO].[SFC Dynamic]    Script Date: 30/09/2022 16:36:10 ******/
SET ANSI_NULLS ON
GO
...
SELECT A.*
	...
	,IIF((
			(
				...
				AND dpo.udf_month_diff(IIF(B.Date_Appointed > Temp2.[Issued Date], B.Date_Appointed, Temp2.[Issued Date]), A.[Collected Date]) > IIF(A.[Collected Date] >= '2021-12-01', 9, 6)
				)
			--AND dpo.udf_month_diff(IIF(Temp2.[Next_Issued_Date] IS NOT NULL,Temp2.[Next_Issued_Date],Temp2.[Issued Date]) ,A.[Collected Date]) > 0
			OR (
