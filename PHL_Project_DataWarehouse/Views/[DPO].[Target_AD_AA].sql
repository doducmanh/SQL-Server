...
/****** Object:  View [DPO].[Target_AD_AA]    Script Date: 30/09/2022 16:37:36 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [DPO].[Target_AD_AA]
AS
SELECT [AD Code]
	,[AD Name]
	,unpvt.[Grade]
	,YearTarget
	,CAST(MonthTarget AS INT) AS MonthTarget
	,TargetAA
	,LS.[Last_Status]
	,LS.[Last_Status_Date]
	,LS.[Appointed_Date]
...
FROM (
	...
	FROM [PowerBI].[DPO].[DP_AD_TARGET_AA]
	WHERE [AD Code] <> '0'
		AND [YEAR] = '2021'
	) p
UNPIVOT(TargetAA FOR MonthTarget IN (
			[1]
			,[2]
			,[3]
			,[4]
...
LEFT JOIN [PowerBI].[DPO].[DW_AD] AS LS ON [AD Code] = LS.[AD_Code]
